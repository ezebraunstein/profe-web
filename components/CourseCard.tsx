    ...
    <>
      <Link href={href} className='w-full border rounded-lg transition shadow-sm hover:shadow-md cursor-pointer'>
        {course.lessons[0]?.video?.publicPlaybackId && (
          <Image
            className="w-full rounded-t-[7px] overflow-hidden"
        ...
        <div className="p-8">
          {!course.published && (<span className="bg-slate-200 text-slate-700 rounded-full text-xs py-1 px-3 mb-2 inline-block">Borrador</span>)}
